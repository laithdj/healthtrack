export function init(Survey: any) {
  const widget = {
    ...
    widgetIsLoaded() {
      // return typeof $ == "function" && !!$.fn.select2; //return true if jQuery and select2 widget are
      // loaded on the page
      return true; // we do not require anything so we just return true.
    },
    // SurveyJS library calls this function for every question to check, if it should use this widget
    // instead of default rendering/behavior
    isFit(question) {
    ...
    // property - it means that it will activated if a property of the existing question type is set to
    // particular value,
    //  for example inputType = "date"
    // type - you are changing the behaviour of entire question type.
    // For example render radiogroup question differently, have a fancy radio buttons
    // customType - you are creating a new type, like in our example "textwithbutton"
    activatedByChanged() {
      // we do not need to check acticatedBy parameter, since we will use our widget for customType only
      // We are creating a new class and derived it from text question type.
      //  It means that text model (properties and fuctions) will be available to us
    ...
    // If you want to use the default question rendering then set
    // this property to true. We do not need any default rendering, we will use our our htmlTemplate
